...
class ModifiedSLDPipeline(SLDPipeline):
    ...
    def _expand_safe_text_embeddings(self, text_embeddings, num_images_per_prompt):
        ...
        concat_safety_embeds = []
        for i in range(0, shape_max_length, max_length):
            concat_safety_embeds.append(
                self.text_encoder(
                    safety_concept_input.input_ids[:, i: i + max_length].to(self.device)
                )[0]
            )
        safety_embeddings = torch.cat(concat_safety_embeds, dim=1)
    ...
    def _new_encode_prompt(self, prompt, num_images_per_prompt, do_classifier_free_guidance, negative_prompt, enable_safety_guidance=True, prompt_ids=None, prompt_embeddings=None):
        ...
        if prompt_embeddings is not None:
            shape_max_length = prompt_embeddings.shape[1] if prompt_embeddings.shape[1] > 77 else 77
            ...
            for i in range(0, shape_max_length, max_length):
                concat_embeds.append(
                    self.text_encoder(
                        text_input_ids[:, i: i + max_length].to(self.device)
                    )[0]
                )
            text_embeddings = torch.cat(concat_embeds, dim=1)
        ...
        if do_classifier_free_guidance:
            ...
            for i in range(0, shape_max_length, max_length):
                concat_unc_embeds.append(
                    self.text_encoder(
                        uncond_input.input_ids[:, i: i + max_length].to(self.device)
                    )[0]
                )
